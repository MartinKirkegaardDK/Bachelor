...
def calculate_betweenness_centrality(filepath):
    # Defining a dictionary to store the country-to-continent mapping
    country_to_continent = {}

    # Opening the all.csv file (which contains the necessary data for the mapping)
    with open('src/Visualisations/Continents/all.csv', newline='', encoding='utf8') as csvfile:
    ...
    # Converting the dictionary to a pd df
    ...
    # Merging the ISO code to continent mapping with the edge list data
    ...
    # Creating a graph and adding the edges from the dataframe
    G = nx.from_pandas_edgelist(df, source='ISO_CODE_1', target='ISO_CODE_2', edge_attr='FBDist')

    # Adding continent information to the nodes in the graph
    for iso_code in G.nodes:
        continents = country_to_continent[iso_code]
        # Converting the list of continents to a tuple if necessary
        if isinstance(continents, list):
            continents = tuple(continents)
        G.nodes[iso_code]['Continent'] = continents

    # Grouping the edges in the graph by continent and calculating the betweenness centrality for each continent
    ...
    # Printing the betweenness centrality for each continent
...
def draw_graph(G, node_size=20, node_color='blue', edge_color='gray', alpha=0.5):
    # Defining the layout of the graph and drawing the graph
